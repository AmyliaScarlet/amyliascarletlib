...
import com.amyliascarlet.lib.utils.Convert;
import com.amyliascarlet.lib.utils.DateUtil;
...
public final class DataTable  {
     ...
     * @version: 2.0
     */
    public DataRow newRow() throws DataTableException {
    ...
    public DataColumn addColumn(String columnName, Type dataType) {
        return this.columns.addColumn(columnName, dataType);
    }

    public DataColumn addColumnIndex(int index, String columnName, Type dataType)  {
...
//                // 如果当前记录时间晚于当前记录时间
//                if (position == 0) {
//                    lastTime = DateUtil.getDates(row.getValue(SCAN_TIME).toString());
//                    result = row.getValue(field.toString());
//                } else {
//                    newDate = DateUtil.getDates(row.getValue(SCAN_TIME).toString());
...
//                } catch (Exception e) {
//                    Log.e("TableCalculate Error：" + e.getMessage());
//                }
//            }
//        }
//        // 循环计算数据行
