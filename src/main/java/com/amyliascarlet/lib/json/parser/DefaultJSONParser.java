...
import static com.amyliascarlet.lib.json.parser.JSONLexer.EOI;
import static com.amyliascarlet.lib.json.parser.JSONToken.*;
...
//            }
            ...
            case EOF:
                ...
                throw new JSONException("unterminated jsontest string, " + lexer.info());
            case HEX:
                byte[] bytes = lexer.bytesValue();
                lexer.nextToken();
                return bytes;
        ...
        try {
            if (lexer.isEnabled(Feature.AutoCloseSource)) {
                if (lexer.token() != JSONToken.EOF) {
                    throw new JSONException("not close jsontest text, token : " + JSONToken.name(lexer.token()));
                }
            }
        } finally {
            lexer.close();
