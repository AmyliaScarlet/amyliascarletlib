package com.amyliascarlet.lib.json.support.jaxrs;
...
public class TestIssue885 extends JerseyTest {
    ...
    protected Application configure() {
        ...
        config.packages("com.amyliascarlet.json");
        return config;
    }

    @Test
    public void testWriteTo() {
        final String user = target("user").request().accept("application/jsontest").get(String.class);
    ...
    public void testWriteToWithPretty() {
        //System.out.println("@@@@@BeanTest Pretty");
        final String user = target("user").queryParam("pretty", "true").request().accept("application/jsontest").get(String.class);
        // {"createdOn":1412036891919,"id":12345,"name":"smallnest"}]
        assertTrue(user.indexOf("createdOn") > 0);
        assertTrue(user.indexOf("\"id\":12345") > 0);
        assertTrue(user.indexOf("\"name\":\"smallnest\"") > 0);
    ...
    public void testReadFrom() {
        final User user = target("user").request().accept("application/jsontest").get(User.class);
        assertNotNull(user);
